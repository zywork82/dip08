...
from db import db  # make sure db.py exports `db` as your MongoClient["dip"]
...
async def update_me(update_data: dict, current_user: dict = Depends(get_current_user)):
    ...
    await db.users.update_one({"_id": ObjectId(current_user["_id"])}, {"$set": payload})
...
async def analytics(current_user: dict = Depends(get_current_user)):
    ...
    pipeline = [
        {"$match": {"user_id": uid}},
        {"$group": {"_id": "$event_type", "count": {"$sum": 1}, "avg_duration": {"$avg": "$duration"}}}
    ]
    ...
    return {"total_events": total, "by_type": by_type, "daily_last_7_days": daily}
