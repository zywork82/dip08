...
# you told me your .env uses GEMINI_API_KEY and GEMINI_IMAGE_MODEL
...
# where we save temp images when tmp=true
TMP_DIR = Path(__file__).with_name("tmp_images")
...
def build_full_skeleton(story: str) -> Dict[str, Any]:
    """
    Build branching skeleton.
    - scenario root
    - D1/D2/D3
    - sub-branches
    - final leaves -> E1/E2/E3 endings
    """
    ...
    # Wire leaves to endings (round robin)
...
def assign_aspects_unique_per_sibling(model, aspects, seed=None):
    """
    Assign 1 psych aspect per sibling among options.
    """
    if seed:
        try:
    ...
    # force defaults for scenario and endings
...
def fill_content_for_nodes(story, node_ids, aspect_map):
    """
    Ask OpenAI to generate:
    - text: the clickable/visible choice
    - narrative: short stakes / consequence / thought prompt
    """
    ...
    def one_batch(batch_ids):
        ...
        sys = (
            ...
            "- 'text' = short clickable choice or decision prompt.\n"
            "- 'narrative' = short consequence, risk, or reflective nudge.\n"
        ...
        - 'narrative' should highlight stakes, impact, or what happens next.
        - No field >120 chars.
        """)
...
def apply_content(model, content_map):
    """
    Merge AI-generated text/narrative back into nodes.
    """
...
def build_flat_with_hubs(model, aspect_map):
    """
    Final shape:
    - keys like "101", "101A", ..., "E1"
    - each node:
      {
        "id": "...",
        "type": "scenario" | "option" | "ending",
        "position": "",
        "data_description": "...",
        "options": [...],
        "psych_dimensions": "..."
      }
    Only E1/E2/E3 are "ending".
    """
    nodes = model["nodes"]
    edges = model["edges"]
    ...
    def fallback_description(node_id: str, is_leaf: bool, chosen_ending: Optional[str]) -> str:
...
# =========================
# === IMAGE GENERATION CORE (YOUR LOGIC, ADAPTED) ===
# =========================

def _fallback_png_real_bytes() -> bytes:
    """
    We generate a valid 1x1 transparent PNG using Pillow.
    This guarantees Windows can open fallback images.
    """
...
def _generate_single_image_bytes(prompt: str) -> Optional[bytes]:
    """
    This is your logic, 1:1 spirit:
    - call Gemini model.generate_content(prompt)
    - gather inline_data.data from response.candidates[0].content.parts
    - inline_data.data is base64 -> decode -> image bytes
    - open with PIL (to validate) and convert to PNG
    - return PNG bytes
    If anything fails → return None, caller will fallback.
    """
    ...
    try:
        ...
        # exactly like your script: one-shot generate_content
        response = model.generate_content(prompt)

        # grab raw inline encoded parts
        parts = []
            ...
            if hasattr(candidate, "content") and hasattr(candidate.content, "parts"):
                for p in candidate.content.parts:
                    if hasattr(p, "inline_data") and p.inline_data:
                        # p.inline_data.data is base64-encoded bytes
                        parts.append(p.inline_data.data)

        if not parts:
            # no inline image found
            return None
        ...
        # validate and normalize with Pillow
...
def generate_image_for_node(node_id: str,
                            data_description: str,
                            save_tmp: bool) -> str:
    """
    Public helper used by /generate_images:
    - Builds the "scene" text from node_id + data_description
    - Calls _generate_single_image_bytes()
    - Falls back to 1x1 PNG if no image
    - Saves tmp/<id>.png if save_tmp is True
    - Returns base64 string for response
    """

    # This is your "scene". We're using data_description as the scene content.
    scene_text = (
        "Create a clean, modern, flat illustration, no text on the image. "
        "Represent the decision/leadership scenario described below. "
        "Scene:\n"
        f"{data_description}\n"
        f"(Node ID: {node_id})"
    )

    img_bytes = _generate_single_image_bytes(scene_text)

    if img_bytes is None:
        # use fallback PNG bytes, but legit PNG so Windows opens it
        img_bytes = _fallback_png_real_bytes()

    # save to disk if tmp=true
    if save_tmp:
...
def generate():
    ...
    Request body:
    {
      ...
      "psych_seed": 42        // optional, to stabilize psych assignment
    }
    """
    ...
    try:
        # 1. build raw skeleton graph
        model = build_full_skeleton(story)

        # 2. assign psych aspects per branch sibling group
        ...
        # 4. apply generated text back to skeleton
        model = apply_content(model, node_content_map)

        # 5. flatten into simple hub/option/ending structure for the front end
        flat = build_flat_with_hubs(model, aspect_map)

        # 6. if download requested, serve as file
        if download_flag:
...
def generate_images():
    ...
    Request body:
    {
      "tmp": true,
      "nodes": [
        {"id": "101", "data_description": "Investigate the claims thoroughly..."},
        {"id": "101A","data_description": "Reassure members privately..."}
      ]
    }
    ...
    {
      "images": [
        {"id": "101", "data_description": "...", "image_b64": "...."},
        {"id": "101A", "data_description": "...", "image_b64": "...."}
      ]
    }
    """
    body = request.get_json(silent=True) or {}
    save_tmp = bool(body.get("tmp", False))
    ...
    for node in nodes:
        ...
        img_b64 = generate_image_for_node(nid, desc, save_tmp)
...
@app.delete("/tmp_images")
def clear_tmp_images():
    """
    Delete all saved temporary PNGs in tmp_images/.
    """
    if TMP_DIR.exists():
        count = 0
